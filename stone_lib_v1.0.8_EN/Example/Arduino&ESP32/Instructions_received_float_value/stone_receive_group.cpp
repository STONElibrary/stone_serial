...
#define case_stoner_data STONER = datainit(STONER.len+1);\
...
//  int temp_value = 0;
  ...
  switch (STONER.cmd){
    ...
    case control_get_xy: {
      ...
      memcpy(STONER.widget,STONE_RX_BUF+7,STONER.len-8);
      STONER.long_value = 0;
      for(int xy=0;xy<8;xy++)
      {
        STONER.long_value =STONER.long_value << 8;
        STONER.long_value += STONE_RX_BUF[7+STONER.len-8+xy];
      }
    }break;
    case control_get_wh: {
      STONER = widgetinit(STONER.len-8+1);
      ...
      {
        STONER.long_value =STONER.long_value << 8;
        STONER.long_value += STONE_RX_BUF[7+STONER.len-8+wh];
      }
    }break;
